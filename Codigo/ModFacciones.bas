...
Public Const ExpAlUnirse = 10000000
Public Const ExpX100 = 5000000
...
If UserList(UserIndex).Faccion.CriminalesMatados < val(GetVar(IniPath & "Config.ini", "FACCIONES", "MatarArmada")) Then
    Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Para unirte a nuestras fuerzas debes matar al menos " & val(GetVar(IniPath & "Config.ini", "FACCIONES", "MatarArmada")) & " criminales, solo has matado " & UserList(UserIndex).Faccion.CriminalesMatados & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
    Exit Sub
End If
...
If UserList(UserIndex).Faccion.CriminalesMatados \ 50 = _
   UserList(UserIndex).Faccion.RecompensasReal Then
    Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Ya has recibido tu recompensa, mata 50 crinales mas para recibir la proxima!!!" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
Else
...
If UserList(UserIndex).Faccion.CiudadanosMatados < val(GetVar(IniPath & "Config.ini", "FACCIONES", "MatarCaos")) Then
    Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Para unirte a nuestras fuerzas debes matar al menos " & val(GetVar(IniPath & "Config.ini", "FACCIONES", "MatarCaos")) & " ciudadanos, solo has matado " & UserList(UserIndex).Faccion.CiudadanosMatados & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
    Exit Sub
End If
...
UserList(UserIndex).Faccion.RecompensasCaos = UserList(UserIndex).Faccion.CiudadanosMatados \ 50

Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Bienvenido a al lado oscuro!!!, aqui tienes tu armadura. Por cada 50 ciudadanos que acabes te dare un recompensa, buena suerte soldado!" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
...
If UserList(UserIndex).Faccion.CiudadanosMatados \ 50 = _
   UserList(UserIndex).Faccion.RecompensasCaos Then
    Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Ya has recibido tu recompensa, mata 50 ciudadanos mas para recibir la proxima!!!" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
Else
    Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "Aqui tienes tu recompensa noble guerrero!!!" & "°" & str(Npclist(UserList(UserIndex).flags.TargetNpc).Char.CharIndex))
    Call AddtoVar(UserList(UserIndex).Stats.Exp, ExpX100, MAXEXP)
    Call SendData(ToIndex, UserIndex, 0, "||Has ganado " & ExpX100 & " puntos de experiencia." & FONTTYPE_FIGHT)
