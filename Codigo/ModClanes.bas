...
If UserList(UserIndex).Stats.ELV < val(GetVar(IniPath & "Config.ini", "CLANES", "lvlCrearClan")) Then
    CanCreateGuild = False
    Call SendData(ToIndex, UserIndex, 0, "||Para fundar un clan debes de ser nivel " & val(GetVar(IniPath & "Config.ini", "CLANES", "lvlCrearClan")) & " o superior" & FONTTYPE_GUILD)
    Exit Function
End If

If UserList(UserIndex).Stats.UserSkills(Liderazgo) < 100 Then
    CanCreateGuild = False
    Call SendData(ToIndex, UserIndex, 0, "||Para fundar un clan necesitas al menos 100 pts en liderazgo" & FONTTYPE_GUILD)
    Exit Function
End If
