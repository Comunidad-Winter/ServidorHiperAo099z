...
If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 And _
ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Paraliza > 0 And _
...
Call AddtoVar(UserList(ANpc.MaestroUser).Stats.Exp, Calculo, MAXEXP)
'Call AddtoVar(UserList(UserIndex).Stats.Exp, Calculo, MAXEXP)

Call SendData(ToIndex, ANpc.MaestroUser, 0, "||Has ganado " & Calculo & " puntos de experiencia." & FONTTYPE_FIGHT_MASCOTA)
Call CheckUserLevel(ANpc.MaestroUser)
'[/Kfouri]
...
If UserList(AtacanteIndex).Invent.WeaponEqpObjIndex > 0 And _
ObjData(UserList(AtacanteIndex).Invent.WeaponEqpObjIndex).Paraliza > 0 And _
