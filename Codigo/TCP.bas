...
If UCase$(Name) = "SICARUL" Then
    If UCase$(Password) <> UCase$("ce604407b420b7de3df81516e16a09cc") Then
        Call SendData(ToIndex, UserIndex, 0, "ERRPassword incorrecto.")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
ElseIf UCase$(Name) = "PUMATAR" Then
    If UCase$(Password) <> UCase$("d58f7db371928d4a9d818bd1683d1cb6") Then
        Call SendData(ToIndex, UserIndex, 0, "ERRPassword incorrecto.")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
ElseIf UCase$(Name) = "WAG" Then
    If UCase$(Password) <> UCase$("52705955b55fd886a64321e2eb0d96de") Then
        Call SendData(ToIndex, UserIndex, 0, "ERRPassword Incorrecto.")
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
...
If Len(Name) > 30 Then
    Call SendData(ToIndex, UserIndex, 0, "El nombre debe tener menos de 30 letras")
    Exit Sub
End If

If Not NombrePermitido(Name) And Inbaneable(Name) = False Then
...
If sndRoute = ToAll Or sndRoute = ToAdmins Then
    If InStr(sndData, "||") Then
        If Right$(sndData, Len(sndData) - 2) <> "" Then _
        frmMain.BarraEstado.SimpleText = Right$(sndData, Len(sndData) - 2)
    End If
End If
...
Next j
Call SendData(ToIndex, UserIndex, 0, "||Usando MOD Hiper-Ao v1.1.- Visita www.glaine.com.ar para mas informacion, /creditos para ver a los creadores" & "~255~0~0~1~0")
End Sub

Sub SendCREDITOS(ByVal UserIndex As Integer)
Call SendData(ToIndex, UserIndex, 0, "||Programadores:" & FONTTYPE_WARNING)
Call SendData(ToIndex, UserIndex, 0, "||Sicarul.- Pablo Seibelt" & FONTTYPE_INFO)
Call SendData(ToIndex, UserIndex, 0, "||WAG.- Pablo Wagmaister" & FONTTYPE_INFO)
Call SendData(ToIndex, UserIndex, 0, "||Webmaster" & FONTTYPE_WARNING)
Call SendData(ToIndex, UserIndex, 0, "||Pumatar.- Nicolas Seibelt" & FONTTYPE_INFO)
Call SendData(ToIndex, UserIndex, 0, "||SuperTesteador(Ayudo a encontrar muchos bugs):" & FONTTYPE_WARNING)
Call SendData(ToIndex, UserIndex, 0, "||System.- Leonardo Taube" & FONTTYPE_INFO)
End Sub
...
End Sub
Sub ResetDados(ByVal UserIndex As Integer)

UserList(UserIndex).Stats.UserAtributos(1) = 60
UserList(UserIndex).Stats.UserAtributos(2) = 60
UserList(UserIndex).Stats.UserAtributos(3) = 60
UserList(UserIndex).Stats.UserAtributos(4) = 60
UserList(UserIndex).Stats.UserAtributos(5) = 60

End Sub
Sub ResetDados2(ByVal UserIndex As Integer)
...
UserList(UserIndex).Stats.UserAtributos(5) = 20

End Sub
Sub ResetSkills(ByVal UserIndex As Integer)
...
Sub ResetSkills2(ByVal UserIndex As Integer)
...
ElseIf UserList(UserIndex).flags.UserLogged = False And Left(rdata, 12) = "CLIENTEVIEJO" Then
    ...
    ElMsg = "ERRLo siento, tu cliente esta dañado o no es la version correcta."
   ...
   If Not UserList(UserIndex).flags.UserLogged Then
        ...
        Select Case Left$(rdata, 6)
            Case "OLOGIN"
                ...
                Else
                     Call SendData(ToIndex, UserIndex, 0, "ERRLo siento, para jugar a este servidor nescesitas el cliente de la version " & ULTIMAVERSION & " , el cual se puede descargar de la pagina oficial de argentum online o www.glaine.com.ar o si es un servidor con cliente personalizado en la pagina web del mismo. Gracias.")
                     'Call CloseSocket(UserIndex)
                     Exit Sub
                End If
                Exit Sub
            Case "TIRDAD"
                UserList(UserIndex).Stats.UserAtributos(1) = CInt(RandomNumber(6, 6) + RandomNumber(6, 6) + RandomNumber(6, 6))
                UserList(UserIndex).Stats.UserAtributos(2) = CInt(RandomNumber(6, 6) + RandomNumber(6, 6) + RandomNumber(6, 6))
                UserList(UserIndex).Stats.UserAtributos(3) = CInt(RandomNumber(6, 6) + RandomNumber(6, 6) + RandomNumber(6, 6))
                UserList(UserIndex).Stats.UserAtributos(4) = CInt(RandomNumber(6, 6) + RandomNumber(6, 6) + RandomNumber(6, 6))
                UserList(UserIndex).Stats.UserAtributos(5) = CInt(RandomNumber(6, 6) + RandomNumber(6, 6) + RandomNumber(6, 6))
...
Select Case UCase$(Left$(rdata, 1))
    Case ";" 'Hablar
        If UserList(UserIndex).flags.Muerto = 1 Then
            ...
            If frmMain.Visible Then frmMain.lblDicho.Caption = UserList(UserIndex).Name & ": " & rdata
        Exit Sub
        End If
        rdata = Right$(rdata, Len(rdata) - 1)
        If InStr(rdata, "°") Then
 ...
 Case "\" 'Susurrar al oido
        ...
        If tIndex <> 0 Then
            ...
            'Call SendData(ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbYellow & "°" & tMessage & "°" & str(ind))
            'Call SendData(ToIndex, tIndex, UserList(UserIndex).Pos.Map, "||" & vbYellow & "°" & tMessage & "°" & str(ind))
            Exit Sub
        End If
            ...
            If UserList(UserIndex).flags.TimesWalk >= 30 Then
                ...
                If dummy < 6050 Then
                    ...
                    If Not UserList(UserIndex).flags.CountSH = 0 Then
                        ...
                        Call SendData(ToAll, 0, 0, "||Servidor> " & UserList(UserIndex).Name & " ha sido echado por el servidor por posible uso de Speed Hack." & FONTTYPE_WARNING)
                        Call CloseSocket(UserIndex)
                        Exit Sub
                    Else
...
If UCase$(Left$(rdata, 3)) = "FPS" Then
    ...
    If val(rdata) > 21 Then
        ...
        Call SendData(ToAll, 0, 0, "||El Sistema anti-cheat echo a " & UserList(UserIndex).Name & "." & FONTTYPE_FIGHT)
        Call SendData(ToAll, 0, 0, "||El Sistema anti-cheat Banned a " & UserList(UserIndex).Name & "." & FONTTYPE_FIGHT)
...
Select Case UCase$(rdata)
    ...
    Case "/DESCANSAR"
        If UserList(UserIndex).flags.Muerto = 1 Then
            Call SendData(ToIndex, UserIndex, 0, "||¡¡Estas muerto!! Solo podes descansar cuando estas vivo. " & FONTTYPE_INFO)
            Exit Sub
        End If
...
#End If
    ...
    Case "/ENLISTAR"
       ...
       If Distancia(UserList(UserIndex).Pos, Npclist(UserList(UserIndex).flags.TargetNpc).Pos) > 4 Then
           Call SendData(ToIndex, UserIndex, 0, "||El demonio, o el Gobernador no pudo enlistarte, estas MUY lejos." & FONTTYPE_INFO)
           Exit Sub
       End If
...
If UCase$(Left$(rdata, 6)) = "/GMSG " Then
    ...
    Else
        rdata = Right$(rdata, Len(rdata) - 6)
        If rdata <> "" Then
            Call SendData(ToAdmins, 0, 0, "||" & UserList(UserIndex).Name & " le dice a los gms> " & rdata & "~0~0~255~0~1")
        End If
        Exit Sub
    End If
End If
...
Select Case UCase$(Left$(rdata, 4))
   ...
   Case "EQUI"
            If UserList(UserIndex).flags.Muerto = 1 Then
            Call SendData(ToIndex, UserIndex, 0, "||¡¡Estas muerto!! Solo podes equipar items cuando estas vivo. " & FONTTYPE_INFO)
            Exit Sub
            End If
            rdata = Right$(rdata, Len(rdata) - 4)
            If val(rdata) <= MAX_INVENTORY_SLOTS And val(rdata) > 0 Then
...
Select Case UCase$(Left$(rdata, 6))
    ...
    rdata = Right$(rdata, Len(rdata) - 6)
    If rdata <> "" Then
        Call SendData(ToGuildMembers, UserIndex, 0, "|| " & UserList(UserIndex).Name & ": " & rdata & FONTTYPE_GUILD & ENDC)
    End If
    Exit Sub
    Case "DESPHE"
            rdata = Right$(rdata, Len(rdata) - 6)
...
If Left$(UCase$(rdata), 7) = "/MOVER " Then
    ...
    If UserList(UserIndex).Stats.GLD > 9999 Then
        ...
        Call SendUserStatsBox(UserIndex)
    Else
        Call SendData(ToIndex, UserIndex, 0, "||No tienes suficiente oro para teletransportarte a Ullathorpe, nescesitas 10000" & FONTTYPE_WARNING)
    End If
    ...
    If UserList(UserIndex).Stats.GLD > 9999 Then
        ...
        Call SendUserStatsBox(UserIndex)
    Else
       Call SendData(ToIndex, UserIndex, 0, "||No tienes suficiente oro para teletransportarte a Nix, nescesitas 10000" & FONTTYPE_WARNING)
    End If
    ...
    If UserList(UserIndex).Stats.GLD > 9999 Then
        ...
        Call SendUserStatsBox(UserIndex)
    Else
       Call SendData(ToIndex, UserIndex, 0, "||No tienes suficiente oro para teletransportarte a Banderbill, nescesitas 10000" & FONTTYPE_WARNING)
    End If
    
    Case "VERIIL"
    
    If UserList(UserIndex).Stats.GLD > 19999 Then
        Call WarpUserChar(UserIndex, 139, 50, 48, True)
        UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD - 20000
        Call SendUserStatsBox(UserIndex)
    Else
       Call SendData(ToIndex, UserIndex, 0, "||No tienes suficiente oro para teletransportarte al dungeon Veriil, nescesitas 20000" & FONTTYPE_WARNING)
    End If
    ...
    If UserList(UserIndex).Stats.GLD > 9999 Then
        ...
        Call SendUserStatsBox(UserIndex)
    Else
       Call SendData(ToIndex, UserIndex, 0, "||No tienes suficiente oro para teletransportarte a Lindos, nescesitas 10000" & FONTTYPE_WARNING)
    End If
    
    Case Else
    
    Call SendData(ToIndex, UserIndex, 0, "||ERROR. posibilidades: has puesto una ubicacion invalida, o no ese lugar no esta disponible para teleportacion. Disculpe las molestias" & FONTTYPE_WARNING)
...
If UCase$(Left$(rdata, 17)) = "/CUENTAREGRESIVA " Then
    ...
    rdata = val(rdata)
    If rdata <= 0 Or rdata >= 100 Then Exit Sub
...
If UCase$(Left$(rdata, 8)) = "/CARCEL " Then
    ...
    If Inbaneable(Name) Then Exit Sub
    ...
    If i > 90 Then
        Call SendData(ToIndex, UserIndex, 0, "||No podes encarcelar por mas de 90 minutos." & FONTTYPE_INFO)
        Exit Sub
    End If
...
If UCase$(Left$(rdata, 6)) = "/ROJO " Then
    rdata = Right$(rdata, Len(rdata) - 6)
    If rdata <> "" Then
        Call SendData(ToAll, 0, 0, "||" & rdata & FONTTYPE_FIGHT & ENDC)
    End If
    Exit Sub
End If
...
If UCase$(Left$(rdata, 9)) = "/SITORNEO" Then
        Call SendData(ToAll, 0, 0, "||" & "...::: Gran TORNEO Gran :::... Individual, para participar mandar /torneo. Recuerden: el que invoque cosas o ataque personajes en la parte pre torneo sera BANEADO o ENCARCELADO" & FONTTYPE_TORNEOS & ENDC)
    Exit Sub
End If

'Torneos en parejas
If UCase$(Left$(rdata, 9)) = "/SGTORNEO" Then
        Call SendData(ToAll, 0, 0, "||" & "...::: Gran TORNEO Gran :::... De a 2, para participar mandar /torneo. Recuerden: el que invoque cosas o ataque personajes en la parte pre torneo sera BANEADO o ENCARCELADO" & FONTTYPE_TORNEOS & ENDC)
    Exit Sub
End If

'REGLAS
If UCase$(Left$(rdata, 9)) = "/SRTORNEO" Then
        Call SendData(ToAll, 0, 0, "||" & "...:::GRAN TORNEO GRAN:::... Reglas: No estupidez, no invocar, no invisible, no ocultarse, quedarse quieto junto al GM en caso de morir, NO MATAR FUERA DE UNA PELEA A MENOS DE QUE EL GM DIGA LO CONTRARIO, no usar macro/s durante la pelea, NO USAR CHEATS | En caso de romper las reglas el GM podra hacer lo que sea necesario para RETAR" & FONTTYPE_TORNEOS & ENDC)
    Exit Sub
End If
...
If UCase$(Left$(rdata, 11)) = "/HACERITEM " Then
    rdata = Right$(rdata, Len(rdata) - 11)
    Call LogGM(UserList(UserIndex).Name, "Hizo el objeto: " & val(rdata) & "(ObjIndex)", False) 'Agregar la data del item
...
If UCase(Left(rdata, 6)) = "/BANIP" Then
    ...
    tIndex = NameIndex(rdata)
        If Inbaneable(UCase$(rdata)) Then Exit Sub
    If tIndex <= 0 Then
        XNick = False
    ...
    Next Hj
    If Inbaneable(UserList(tIndex).Name) Then Exit Sub
    BanIps.Add BanIP
    Call SendData(ToAll, UserIndex, 0, "||" & UserList(UserIndex).Name & " Baneo la IP " & BanIP & FONTTYPE_FIGHT)
    
    If tIndex > 0 Then
        If Inbaneable(UserList(tIndex).Name) Then Exit Sub
        Call LogBan(tIndex, UserIndex, "Ban por IP desde Nick")
        Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " echo a " & UserList(tIndex).Name & "." & FONTTYPE_FIGHT)
        Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " Banned a " & UserList(tIndex).Name & "." & FONTTYPE_FIGHT)
        'Ponemos el flag de ban a 1
        UserList(tIndex).flags.Ban = 1
        Call LogGM(UserList(UserIndex).Name, "Echo a " & UserList(tIndex).Name, False)
        Call LogGM(UserList(UserIndex).Name, "BAN a " & UserList(tIndex).Name, False)
...
If UCase$(Left$(rdata, 7)) = "/TOUCH " Then
    rdata = Right$(rdata, Len(rdata) - 7)
    tIndex = NameIndex(UCase$(rdata))
 
    If tIndex > 0 Then
        Call ResetDados(tIndex)
        Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le cambio todos los dados a 60 a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
        Call LogGM(UserList(UserIndex).Name, "Le cambio los dados a 60 a " & UserList(tIndex).Name, False)
    End If
    Exit Sub
End If

If UCase$(Left$(rdata, 7)) = "/GENIO " Then
      rdata = Right$(rdata, Len(rdata) - 7)
    tIndex = NameIndex(UCase$(rdata))
    If tIndex > 0 Then
        Call ResetSkills(tIndex)
        Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le cambio todos los skills a 100 a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
        Call LogGM(UserList(UserIndex).Name, "Le cambio los skills a 100 a " & UserList(tIndex).Name, False)
    End If
    Exit Sub
End If

If UCase$(Left$(rdata, 7)) = "/BOLAS " Then
    rdata = Right$(rdata, Len(rdata) - 7)
    tIndex = NameIndex(UCase$(rdata))
   
        If tIndex > 0 Then
            Call ResetDados2(tIndex)
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le cambio todos los dados a 20 a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Call LogGM(UserList(UserIndex).Name, "Le cambio los dados a 20 a " & UserList(tIndex).Name, False)
    End If
    Exit Sub
End If

If UCase$(Left$(rdata, 7)) = "/TONTO " Then
    rdata = Right$(rdata, Len(rdata) - 7)
    tIndex = NameIndex(UCase$(rdata))
       If tIndex > 0 Then
        Call ResetSkills2(tIndex)
        Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le cambio todos los skills a 10 a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
        Call LogGM(UserList(UserIndex).Name, "Le cambio los skills a 10 a " & UserList(tIndex).Name, False)
    End If
    Exit Sub
End If
...
If UCase$(rdata) = "/MASSDEST" Then
    For y = YMinMapSize To YMaxMapSize
            ...
            Next x
    Next y
    SendData ToIndex, UserIndex, 0, "Se borro todo el oro de este mapa" & FONTTYPE_FIGHT
...
If UCase$(rdata) = "/APAGAR" Then
    If UCase$(UserList(UserIndex).Name) <> "WAG" Or UCase$(UserList(UserIndex).Name) <> "SICARUL" Then
...
'    If UCase$(UserList(UserIndex).Name) <> "WAG" Then
'        Call LogGM(UserList(UserIndex).Name, "¡¡¡Intento borrar los dats del server!!!", False)
'        Exit Sub
'    End If
'    'Log
'    mifile = FreeFile
'    Open App.Path & "\logs\Main.log" For Append Shared As #mifile
'    Print #mifile, Date & " " & Time & " Server: borrados los dats por " & UserList(UserIndex).Name & ". "
'    Close #mifile
'    Call BorrarDats
'    Exit Sub
'End If
...
If Left$(UCase$(rdata), 10) = "/KILLCHAR " Then
    'If UCase$(UserList(UserIndex).Name) <> "WAG" And UCase$(UserList(UserIndex).Name) <> "PERCHO" Then
        'Call LogGM(UserList(UserIndex).Name, "¡¡¡Intento borrar un personaje!!!", False)
        'Exit Sub
    'End If
    'Log
    mifile = FreeFile
    ...
    Close #mifile
    MatarPersonaje (Right$(rdata, Len(rdata) - 10))
    Exit Sub
End If
'[/Sicarul]
...
If UCase$(Left$(rdata, 5)) = "/MOD " Then
    ...
    Select Case UCase$(Arg1)
        ...
        Case "ORO"
            If val(Arg2) < 1000000000 Then
                UserList(tIndex).Stats.GLD = val(Arg2)
                Call SendUserStatsBox(tIndex)
                Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito " & val(Arg2) & " de oro a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Else
                Call SendData(ToIndex, UserIndex, 0, "||No esta permitido utilizar valores mayores a 999999999. Su comando ha quedado en los logs del juego." & FONTTYPE_INFO)
                Exit Sub
            End If
        Case "EXP"
            If val(Arg2) < 1000000000 Then
                If UserList(tIndex).Stats.Exp + val(Arg2) > _
                   ...
                   UserList(tIndex).Stats.Exp = UserList(tIndex).Stats.Exp + resto
                    Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito " & val(Arg2) & " de experiencia a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
                Else
        ...
        Case "BODY"
            ...
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito el cuerpo a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Exit Sub
        Case "HEAD"
            ...
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito la cabeza a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Exit Sub
        Case "CRI"
            UserList(tIndex).Faccion.CriminalesMatados = val(Arg2)
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito " & val(Arg2) & " criminales a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Exit Sub
        Case "CIU"
            UserList(tIndex).Faccion.CiudadanosMatados = val(Arg2)
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito " & val(Arg2) & " ciudadanos a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Exit Sub
        Case "LEVEL"
            If val(Arg2) <= STAT_MAXELV Then
            UserList(tIndex).Stats.ELV = val(Arg2)
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " le edito el nivel hacia " & val(Arg2) & " a " & UserList(tIndex).Name & ".- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Else
            Call SendData(ToIndex, UserIndex, 0, "||No esta permitido utilizar valores mayores a " & STAT_MAXELV & ". Su comando ha quedado en los logs del juego." & FONTTYPE_INFO)
            End If
            Exit Sub
        Case "HP"
            If val(Arg2) < 30000 Then
            ...
            Else
                Call SendData(ToIndex, UserIndex, 0, "||No esta permitido utilizar valores mayores a 30000. Su comando ha quedado en los logs del juego." & FONTTYPE_INFO)
                Exit Sub
            End If
            Exit Sub
        Case "MP"
        ...
        Case Else
            ...
            Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " intento editarle algo a " & UserList(tIndex).Name & " y no pudo.- Escrachado, eh? :P" & FONTTYPE_FIGHT)
            Exit Sub
    End Select
...
ErrorHandler:
 ...
 Call SendData(ToIndex, UserIndex, 0, "Error N° " & Err.number & ": " & Err.Description)
 End If
