...
'                Call ResetUserSlot(i)
            ...
            End If
        End If
        'If UserList(i).Counters.AntiSH > 8 Then
        'UserList(i).Counters.AntiSH2 = UserList(i).Counters.AntiSH2 + 1
        'Call SendData(ToIndex, i, 0, "||Contador SH: " & UserList(i).Counters.AntiSH2 & FONTTYPE_VENENO)
        'If UserList(i).Counters.AntiSH2 > 9 Then
         '   UserList(i).flags.Ban = 1
          '  Call SendData(ToAll, 0, 0, "|| '" & UserList(i).Name & "' fue automaticamente baneado por el servidor por uso de SPEEDHACK" & FONTTYPE_VENENO)
            'Call CloseSocket(i)
        'End If
    ...
    If CuentaRegresiva > 0 Then
        If CuentaRegresiva = 1 Then
            Call SendData(ToAll, 0, 0, "||YA!!!!" & FONTTYPE_GUILD)
        Else
